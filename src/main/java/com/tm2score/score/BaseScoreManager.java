...
public class BaseScoreManager {
     ...
     *
     *
     * int[0] = testkeys scored. int[1] = testevents scored. int[2] = testkeys
     * placed in pending status int[3] = testevents placed in pending status
     * int[4] = testevents skipped because they are incompleted (only if
     * partiallyCompleteBatteriesOk=true) int[5] = returned to TestKey completed
     * because hasUnscored.
     *
     * @param tk
     * @return
    ...
    public void initiateAiMetaScores(TestKey tk, boolean forceRedo) throws Exception
    {
        ...
        try
        {
            if (tk.getJobId() > 0)
            {
                ...
                {
                    LogService.logIt("BaseScoreManager.initiateAiMetaScores() EvalPlan is inactive. Ignoring. evalPlanId=" + evalPlan.getEvalPlanId() + ", testKeyId=" + tk.getTestKeyId());
                    return;
                }
                ...
                if (useThread)
                {
                    LogService.logIt("BaseScoreManager.initiateAiMetaScores() Submitting EvalPlan for scoring via thread. TestKeyId=" + tk.getTestKeyId() + ", userId=" + tk.getUserId());
                    new Thread(epst).start();
                } else
                {
                    LogService.logIt("BaseScoreManager.initiateAiMetaScores() DDD.1 Submitting EvalPlan for scoring inline. TestKeyId=" + tk.getTestKeyId() + ", userId=" + tk.getUserId());
                    boolean result = epst.submitEvalPlanScore();
                    LogService.logIt("BaseScoreManager.initiateAiMetaScores() DDD.2 BACK from EvalPlan scoring call. result=" + result + ", TestKeyId=" + tk.getTestKeyId() + ", userId=" + tk.getUserId());
                    if (result)
                        Thread.sleep(500);
                }
            } else
            {
                String reportRuleAiMetaScoreTypeIdStr;

                if (tk.getJobDescripId() > 0)
                    reportRuleAiMetaScoreTypeIdStr = Integer.toString(AiMetaScoreType.JOBDESCRIP.getAiMetaScoreTypeId());
                ...
                for (String ss : reportRuleAiMetaScoreTypeIdStr.split(","))
                {
                    if (ss.isBlank())
                        continue;
                    ...
                    {
                        String jobDescripIdStr = tk.getJobDescripId() > 0 ? Integer.toString(tk.getJobDescripId()) : null;
        ...
        {
            LogService.logIt(e, "BaseScoreManager.initiateAiMetaScores() ");
            throw e;
        }
    }
