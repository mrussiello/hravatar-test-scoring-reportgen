...
/**
 *
 * NOTE: This is needed to ensure that all database transactions commit before a
 * given record in the DBMS is read again.
 *
 *
 * @author Mike
 */
public class ScoreManager extends BaseScoreManager {
    ...
    public int[] rescoreTestKey(long testKeyId, boolean rescoreOnly, boolean clearExternal) throws Exception
    {
        try
        {
            ...
            if (!BaseScoreManager.isScoringFirstTimeOrRepeatAllowed(testKeyId))
                throw new STException("g.PassThru", new String[]
                {
                    "TestKey has been active in scoring system in last 5 minutes. Please wait and try again. testKeyId=" + testKeyId
                });
     ...
     * partiallyCompleteBatteriesOk=true) int[5] = incomplete battery test
     * events scored int[6] = incomplete battery test events in pending. int[7]
     * = testkeys returned to completed because they have unscored testevents
     * (not pending, probably waiting on proctoring)
     *
     * @return
     * @throws Exception
     */
    ...
    public boolean resetTestKeyStatusForScoring(TestKey tk, boolean fullReset, boolean clearExternal) throws Exception
    {
        try
        {
            ...
            {
                // if not completed, reset TestKeyStatus to started and throw ScoringException;
                if (te.getTestEventStatusTypeId() < TestEventStatusType.COMPLETED.getTestEventStatusTypeId())
                {
