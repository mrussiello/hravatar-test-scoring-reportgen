...
public class BaseAudioSampleAvItemScorer extends BaseAvItemScorer implements AvItemScorer {
        ...
        else
        {
            ...
            if( iir.getSpeechTextStatusType().isCompleteOrPermanentError() || iir.getSpeechTextStatusType().isNotRequired() )   // MJR CHANGED 05052018!!! Was if( iir.getSpeechTextStatusType().isComplete() ). Changed because speechtext errors were preventing video from appearing.
            {
                ...
                if( s2tr!=null )
                {
                    ...
                    speechText = s2tr.getConcatTranscript(0, "" );
                    ...
                    if( iir.getAvItemEssayStatusType().requiresEssayScore() )
                    {
                        ...
                        {
                            doEssayScore(speechText, iir, intnObj );
                     ...
                     *    data[4] = total words
                     */
                    int[] errs = LocalEssayScoringUtils.getWritingErrorCount(speechText, locale, teIpCountry, getWordsToIgnoreLc() );
                    ...
                    */
                    ...
                    featuresScore = 100*computeSpeechTextQualityFactor( speechText, locale, ers, iir.getConfidence() );  // 0 - 100f
            ...
            {
                VoiceVibesResult vvr = new VoiceVibesResult( iir.getVoiceVibesResponseStr(), Locale.US, StringUtils.numWords( speechText==null ? "" : speechText ) );
    ...
    public TextAndTitle getItemScoreTextTitle(int includeItemScoreTypeId, ScorableResponse sr )
    {
        ...
        // For this method, only return a text/title if response included.
        if( !iist.isResponseOrResponseCorrect() || questionText==null || questionText.isBlank() || speechText==null || speechText.isBlank() )
            return null;

        String title = getTextAndTitleIdentifier( sr, intn );   // UrlEncodingUtils.decodeKeepPlus( sr.getExtItemId() );

        // LogService.logIt( "BaseAudioSampleAvItemScorer.getItemScoreTextTitle() title=" + title + ", includeItemScoreTypeId=" + includeItemScoreTypeId + ", intn.unique=" + (this.intn==null ? "null" : this.intn.getUniqueid()) );
        
        title = (title==null ? "" : title + ": ") + questionText;

        String text = speechText;

        // LogService.logIt( "BaseAudioSampleAvItemScorer.getItemScoreTextTitle() title=" + title + ", text=" + text );
