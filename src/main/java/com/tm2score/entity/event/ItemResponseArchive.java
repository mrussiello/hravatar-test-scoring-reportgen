...
@NamedQueries( {
        @NamedQuery( name = "ItemResponseArchive.findByTestEventId", query = "SELECT o FROM ItemResponseArchive AS o WHERE o.testEventId=:testEventId" )
} )
public class ItemResponseArchive implements Comparable<ItemResponseArchive>, Serializable
{
    @Transient
    ...
    public String toString() {
        return "ItemResponseArchive{" + "itemResponseId=" + itemResponseId + ", testEventId=" + testEventId + ", simletId=" + simletId + ", simletVersionId=" + simletVersionId + ", simletAid=" + simletAid + ", simletNodeId=" + simletNodeId + ", simletNodeUniqueId=" + simletNodeUniqueId + '}';
    }
    ...
    public ItemResponse getItemResponse()
    {
        ...
        ir.setArchive(true);
        ir.setClickCount(this.clickCount);
        ir.setCompetencyScoreId( this.competencyScoreId );
        ir.setCorrect( this.correct );
        ir.setCorrectSubnodeSeqIds(correctSubnodeSeqIds);
        ir.setDisplayOrder(displayOrder);
        ir.setIdentifier(identifier);
        ir.setItemParadigmTypeId(itemParadigmTypeId);
        ir.setItemResponseId(-999);
        ir.setItemResponseTypeId(itemResponseTypeId);
        ir.setItemScore(itemScore);
        ir.setMetascore1(metascore1);
        ir.setMetascore2(metascore2);
        ir.setMetascore3(metascore3);
        ir.setMetascore4(metascore4);
        ir.setMetascore5(metascore5);
        ir.setMetascore6(metascore6);
        ir.setMetascore7(metascore7);
        ir.setMetascore8(metascore8);
        ir.setMetascore9(metascore9);
        ir.setRadioButtonGroupId(radioButtonGroupId);
        ir.setRepeatItemSimNodeSeq(repeatItemSimNodeSeq);
        ir.setResponseLevelId(responseLevelId);
        ir.setResponseTime(responseTime);
        ir.setAccessibleForm(accessibleForm);
        ir.setScoreParam1(scoreParam1);
        ir.setScoreParam2(scoreParam2);
        ir.setScoreParam3(scoreParam3);
        ir.setSelectedSubFormatTypeIds(selectedSubFormatTypeIds);
        ir.setSelectedSubnodeSeqIds(selectedSubnodeSeqIds);
        ir.setSelectedValue(selectedValue);
        ir.setShowCount(this.showCount);
        ir.setSimCompetencyId(simCompetencyId);
        ir.setSimId(simId);
        ir.setSimNodeSeq(simNodeSeq);
        ir.setSimVersionId(simVersionId);
        ir.setSimletAid(simletAid);
        ir.setSimletId(simletId);
        ir.setSimletItemTypeId(simletItemTypeId);
        ir.setSimletNodeId(simletNodeId);
        ir.setSimletNodeSeq(simletNodeSeq);
        ir.setSimletNodeUniqueId(simletNodeUniqueId);
        ir.setSimletSubnodeSeq(simletSubnodeSeq);
        ir.setSimletVersionId(simletVersionId);
        ir.setSubnodeFormatTypeId(subnodeFormatTypeId);
        ir.setSubnodeSeq(subnodeSeq);
        ir.setSurveyEventId(surveyEventId);
        ir.setTestEventId(testEventId);
        ir.setTrueScore(trueScore);
        return ir;
        
    }

    @Override
    public int compareTo(ItemResponseArchive o) {
