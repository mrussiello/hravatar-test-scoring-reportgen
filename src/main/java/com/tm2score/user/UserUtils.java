...
public class UserUtils extends FacesUtils
{
    ...
    public String processRegenReportsSimSimVersionId()
    {
        try
        {
            ...
            List<Long> tel = eventFacade.getTestEventIdsForSimIdAndOrOrg(simId, simVersionId, orgId, suborgId, minTestEventId, TestEventStatusType.REPORT_COMPLETE.getTestEventStatusTypeId(), TestEventStatusType.REPORT_COMPLETE.getTestEventStatusTypeId(), new int[]{TestEventStatusType.REPORT_ERROR.getTestEventStatusTypeId()}, start, end, 1000, 0);
            ...
            for( Long testEventId : testEventIdList )
            {
                ...
                if( locStr!= null && !locStr.isEmpty() )
                {
                    try
                    {
                        ...
                        LogService.logIt( "UserUtils.processGenRegenReportsTestEventId() input localeStr=" + locStr + ", converted to a locale=" + loc.toString() +", loc.language=" + loc.getLanguage() );
                    }

                    catch( Exception e )
                    {
                        LogService.logIt( e, "UserUtils.processGenRegenReportsTestEventId() localeStr was " + locStr );
                        loc = null;
                    }
                }
                ...
                if( loc==null )
                {
                    ...
                    if( rl!=null && !rl.isEmpty()  )
                    {
                        ...
                        setStringErrorMessage( "Did not generate any reports. " );
                }
    ...
    public List<String[]> getStatusMap()
    {
        ...
        {
            out.add( new String[] { k, sm.get(k).toString() } );
        }
   ...
   */
    ...
    */    
    ...
    public void rescoreTestEvents( List<Long> tel, String simDescripXml, boolean isThread, boolean percentilesOnly, boolean clearExternal, boolean skipVersionCheck, boolean resetSpeechText) throws Exception
    {
            ...
            for( Long teid : tel )
            {
                LogService.logIt( "UserUtils.rescoreTestEvents() (re)scoring Test EventId=" + teid );
