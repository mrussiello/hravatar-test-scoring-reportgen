 ...
 * @author Mike
 */
public class FindlyDataEntryScoreInfo extends BaseFindlyScoreInfo implements FindlyScoreInfo
{
    ...
    public void populateTestEventAndCreateTestEventScoreList( TestEvent te) throws Exception
    {
        try
        {
            ...
            List<FindlyCompetency> scaleScores = getDataEntryScaleScores();

            // LogService.logIt( "FindlyDataEntryScoreInfo.populateTestEventAndCreateTestEventScoreList() Found " + scaleScores.size() + " Scale Scores.");
    ...
    public List<FindlyCompetency> getDataEntryScaleScores() throws Exception
    {
        ...
        FindlyCompetency fc = getFindlyCompetency( "GrossKPH", "g.FindlyDataEntryGrossKeystrokesPerHour" );
        if( fc != null )
            out.add( fc );

        fc = getFindlyCompetency( "Errors", "g.FindlyDataEntryGrossErrors" );
        if( fc != null )
            out.add( fc );

        fc = getFindlyCompetency( "AdjErrors", "g.FindlyDataEntryAccuracyAdjustedErrors" );
        if( fc != null )
            out.add( fc );

        fc = getFindlyCompetency( "NetKPH", "g.FindlyDataEntryKeystrokesPerHour" );
        if( fc != null )
            out.add( fc );

        fc = getFindlyCompetency( "AccuracyRate", "g.FindlyDataEntryAccuracyRate" );
        if( fc != null )
            out.add( fc );
